...
Mesh* Shape2DFactory::Line(const vec3& point1, const vec3& point2) {
  auto mesh = new Mesh();
  ;
  mesh->mode_ = GL_LINES;
...
Mesh* Shape2DFactory::RectMesh(const vec3& offset, const vec2& size) {
  ...
  mesh->texCoords_.push_back(vec2(0, 0));
  mesh->texCoords_.push_back(vec2(1, 0));
  mesh->texCoords_.push_back(vec2(1, 1));
  mesh->texCoords_.push_back(vec2(0, 1));
  return mesh;
}
