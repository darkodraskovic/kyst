...
MazeFactory::MazeFactory(int width, int height) {
...
const vector<vec3>& MazeFactory::GetVertexArray() {
...
struct compareIVec2 {
  ...
  bool operator()(const ivec2& iv) { return iv.x == key.x && iv.y == key.y; }
};

void MazeFactory::RandomWalk() {
  srand(time(NULL) + MazeFactory::seed_++);
