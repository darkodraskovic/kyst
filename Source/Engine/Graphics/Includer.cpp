...
std::string Includer::Include(std::string path, std::string includeIndentifier)
{
    includeIndentifier += ' ';
    ...
    while (std::getline(file, lineBuffer))
    {
        ...
        if (lineBuffer.find(includeIndentifier) != lineBuffer.npos)
        {
            ...
            fullSourceCode += Include(lineBuffer);
...
void Includer::GetFilePath(const std::string& fullPath, std::string& pathWithoutFileName)
{
